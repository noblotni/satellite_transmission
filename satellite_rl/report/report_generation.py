...
def generate_report(csv_file_path: str, report_path: str, report_title: str, params: dict):
    ...
    graph4 = px.line(
        df_episode_reward, x=df_episode_reward.index, y="reward", title="Reward vs. Episode"
    )
    graph5 = px.line(
        df_episode_modem,
        x=df_episode_modem.index,
        y="nb_modem_min",
        title="nb_modem_min vs. Episode",
    )
    graph6 = px.line(
        df_episode_group,
        x=df_episode_group.index,
        y="nb_group_min",
    ...
    with open(report_path, "w", encoding="utf-8") as report_file:
        ...
        report_file.write(
            '<table style="border-collapse: collapse; width: 35%; margin: 0 auto; text-align: center;">'
        )
        report_file.write(
            '<tr><th style="border: 2px solid black; padding: 8px; font-weight: bold;">Experimental design</th><th style="border: 2px solid black; padding: 8px; font-weight: bold;;">Value</th></tr></thead>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of links</td><td style="border: 1px solid black; padding: 8px;">{params["Number of links"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of modems</td><td style="border: 1px solid black; padding: 8px;">{params["Number of modems"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of groups</td><td style="border: 1px solid black; padding: 8px;">{params["Number of groups"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Time</td><td style="border: 1px solid black; padding: 8px;">{params["Time"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of episodes</td><td style="border: 1px solid black; padding: 8px;">{params["Number of episodes"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of timesteps</td><td style="border: 1px solid black; padding: 8px;">{params["Number of timesteps"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of runs</td><td style="border: 1px solid black; padding: 8px;">{params["Number of runs"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Time out</td><td style="border: 1px solid black; padding: 8px;">{params["Time out"]}</td></tr></table>'
        )
...
def create_graph_lines(dfs, y, title, names=None):
    ...
    fig.update_layout(
        ...
        legend_title="Runs",
        legend=dict(
            orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, traceorder="normal"
        ),
    )
    return fig


def generate_report_runs(
    list_csv_file_path: list[str], report_path: str, report_title: str, params: dict
):
    dfs = [
    ...
    with open(report_path, "w", encoding="utf-8") as report_file:
        ...
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of modems (min)</td><td style="border: 1px solid black; padding: 8px;">{params["Number of modems"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Number of groups (min)</td><td style="border: 1px solid black; padding: 8px;">{params["Number of groups"]}</td></tr>'
        )
        report_file.write(
            f'<tr><td style="border: 1px solid black; padding: 8px;">Time</td><td style="border: 1px solid black; padding: 8px;">{params["Time"]}</td></tr>'
        )
...
def generate_report_comparison(
    list_csv_file_path_actor: list[str],
    list_csv_file_path_ppo: list[str],
    report_path: str,
    report_title: str,
    params: dict,
):
    ...
    graphs_modem_timestep = create_graph_lines(
        list_dfs, "nb_modem_min", "nb_modem_min vs. Timestep", names
    )
    graphs_group_timestep = create_graph_lines(
        list_dfs, "nb_group_min", "nb_group_min vs. Timestep", names
    )

    graphs_reward_episode = create_graph_lines(df_episode_reward, None, "Reward vs. Episode")
    graphs_modem_episode = create_graph_lines(
        df_episode_modem, None, "nb_modem_min vs. Episode", names
    )
    graphs_group_episode = create_graph_lines(
